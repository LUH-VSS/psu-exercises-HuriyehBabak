...
def program(token_stream, parse_tree=False):
    #  PREDICT(NT(program) -> [NT(decl_list), T(EOF)]): {T(FUNC), T(EOF)}
    if token_stream.peek() in ["FUNC", "EOF"]:
...
def stmt_list(token_stream, parse_tree=False):
    #  PREDICT(NT(stmt_list) -> [NT(stmt), NT(stmt_list)]): {T(FOR), T(IDENT), T(MUL), T(BREAK), T(CONTINUE), T(SUB), T(NOT), T(LPAR), T(ET), T(IF), T(RETURN), T(LITERAL), T(WHILE), T(VAR)}
...
def stmt(token_stream, parse_tree=False):
    ...
    if token_stream.peek() in ["VAR"]:
        ...
        if not parse_tree:
            return var_0
        return ["stmt", var_0]
    #  PREDICT(NT(stmt) -> [NT(expr), T(SEMICOLON)]): {T(IDENT), T(ET), T(MUL), T(SUB), T(LITERAL), T(NOT), T(LPAR)}
    if token_stream.peek() in ["IDENT", "ET", "MUL", "SUB", "LITERAL", "NOT", "LPAR"]:
...
def expr(token_stream, parse_tree=False):
    #  PREDICT(NT(expr) -> [NT(term), NT(expr_tail)]): {T(IDENT), T(ET), T(MUL), T(SUB), T(LITERAL), T(NOT), T(LPAR)}
    if token_stream.peek() in ["IDENT", "ET", "MUL", "SUB", "LITERAL", "NOT", "LPAR"]:
...
def if_tail(token_stream, parse_tree=False):
    #  PREDICT(NT(if_tail) -> [E]): {T(FOR), T(IDENT), T(MUL), T(BREAK), T(CONTINUE), T(SUB), T(NOT), T(LPAR), T(ET), T(IF), T(RBRACE), T(RETURN), T(LITERAL), T(WHILE), T(VAR)}
    if token_stream.peek() in [
        "FOR",
        "IDENT",
        "MUL",
...
def term(token_stream, parse_tree=False):
    #  PREDICT(NT(term) -> [NT(factor), NT(term_tail)]): {T(IDENT), T(ET), T(MUL), T(SUB), T(LITERAL), T(NOT), T(LPAR)}
    if token_stream.peek() in ["IDENT", "ET", "MUL", "SUB", "LITERAL", "NOT", "LPAR"]:
...
def expr_tail(token_stream, parse_tree=False):
    #  PREDICT(NT(expr_tail) -> [E]): {T(SEMICOLON), T(RPAR), T(COMMA)}
    if token_stream.peek() in ["SEMICOLON", "RPAR", "COMMA"]:
        # Skip Epsilon
        if not parse_tree:
            return lambda x: x
        return ["expr_tail"]
    ...
    #  PREDICT(NT(expr_tail) -> [NT(bool_expr_tail)]): {T(EQ), T(GE), T(LE)}
    if token_stream.peek() in ["EQ", "GE", "LE"]:
...
def term_tail(token_stream, parse_tree=False):
    ...
    if token_stream.peek() in ["DIV"]:
        ...
        return ["term_tail", var_0, var_1, var_2]
    #  PREDICT(NT(term_tail) -> [E]): {T(SEMICOLON), T(RPAR), T(SUB), T(ADD), T(ASSIGN), T(GE), T(COMMA), T(EQ), T(LE)}
    if token_stream.peek() in ["SEMICOLON", "RPAR", "SUB", "ADD", "ASSIGN", "GE", "COMMA", "EQ", "LE"]:
        # Skip Epsilon
        if not parse_tree:
            return lambda x: x
        return ["term_tail"]
...
def factor_tail(token_stream, parse_tree=False):
    ...
    #  PREDICT(NT(factor_tail) -> [E]): {T(MUL), T(SEMICOLON), T(RPAR), T(COMMA), T(SUB), T(ADD), T(ASSIGN), T(GE), T(DIV), T(EQ), T(LE)}
    if token_stream.peek() in ["MUL", "SEMICOLON", "RPAR", "COMMA", "SUB", "ADD", "ASSIGN", "GE", "DIV", "EQ", "LE"]:
        # Skip Epsilon
        if not parse_tree:
            return lambda x: x
        return ["factor_tail"]
...
def arg_list(token_stream, parse_tree=False):
    #  PREDICT(NT(arg_list) -> [NT(expr), NT(arg_list_tail)]): {T(IDENT), T(ET), T(MUL), T(SUB), T(LITERAL), T(NOT), T(LPAR)}
    if token_stream.peek() in ["IDENT", "ET", "MUL", "SUB", "LITERAL", "NOT", "LPAR"]:
        var_0 = expr(token_stream, parse_tree)
