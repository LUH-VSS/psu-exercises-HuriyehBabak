...
class CodeGeneration(Visitor):
    def __init__(self) -> None:
        super().__init__()
        # {{BEGIN(A4)}}
        # TODO: Einen Stack aktueller Schleifen anlegen
        # {{END(A4)}}
        # {{BEGIN(L4)}}
        self.loop_stack = []
        # {{END(L4)}}
    ...
    def visit_WhileStmt(self, stmt: AST.types.WhileStmt) -> None:
        ...
        # {{BEGIN(A4)}}
        # TODO: Bevor wir Code für den Schleifenkörper ausführen, müssen wir
        #        uns für break und continue merken, wohin gesprungen
        #        werden muss. Dazu sollte diese Klasse einen Stack
        #        der aktiven Schleifen bekommen.
        # {{ELIF(L4)}}
        self.loop_stack.append((loop_header, after_loop))
        # {{END(L4)}}
    ...
    # {{BEGIN(A4)}}
    def visit_BreakStmt(self, stmt: AST.types.BreakStmt) -> None:
        raise NotImplementedError("break not implemented")

    # {{ELIF(L4)}}

    def visit_BreakStmt(self, stmt: AST.types.BreakStmt) -> None:
        loop_header, after_loop = self.loop_stack[-1]
        self.current_block.append(Goto, after_loop.label)
        self.current_block = self.current_function.create_block()

    # {{END(L4)}}

    # {{BEGIN(A4)}}
    def visit_ContinueStmt(self, stmt: AST.types.ContinueStmt) -> None:
        raise NotImplementedError("continue not implemented")
    ...
    # {{BEGIN(A4)}}
    def visit_ForStmt(self, stmt: AST.types.ForStmt) -> None:
    ...
    def visit_ForStmt(self, stmt: AST.types.ForStmt) -> None:
        loop_header = self.current_function.create_block()
        loop_body = self.current_function.create_block()
        loop_body_next = self.current_function.create_block()
        after_loop = self.current_function.create_block()
        ...
        self.loop_stack.append((loop_body_next, after_loop))
        self.current_block = loop_body
        self.visit(stmt.body)
        self.current_block.append(Goto, loop_body_next.label)
        self.loop_stack.pop(-1)

        self.current_block = loop_body_next
        self.visit(stmt.next)
        self.current_block.append(Goto, loop_header.label)
