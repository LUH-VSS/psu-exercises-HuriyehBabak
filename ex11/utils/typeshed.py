    ...
    if expected_type.__args__:
        ...
        if check_args:
            ...
            if unfulfilled_kwonlyargs:
                raise TypeError(
                    "callable passed as {} has mandatory keyword-only arguments in its " "declaration: {}".format(
                        argname, ", ".join(unfulfilled_kwonlyargs)
                    )
                )
            ...
            has_varargs = any(
                param for param in signature.parameters.values() if param.kind == Parameter.VAR_POSITIONAL
            )
...
def check_typevar(
    argname: str, value, typevar: TypeVar, memo: Optional[_CallMemo], subclass_check: bool = False
) -> None:
    if memo is None:
    ...
    if bound_type is None:
        ...
        if typevar.__constraints__ and value_type not in typevar.__constraints__:
            ...
            raise TypeError(
                "{} must be one of ({}); got {} instead".format(subject, typelist, qualified_name(value_type))
            )
    elif typevar.__covariant__ or typevar.__bound__:
        if not issubclass(value_type, bound_type):
            raise TypeError(
                "{} must be {} or one of its subclasses; got {} instead".format(
        ...
        if not issubclass(bound_type, value_type):
            raise TypeError(
                "{} must be {} or one of its superclasses; got {} instead".format(
                    subject, qualified_name(bound_type), qualified_name(value_type)
                )
            )
    else:  # invariant
        if value_type is not bound_type:
            raise TypeError(
                "{} must be exactly {}; got {} instead".format(
                    subject, qualified_name(bound_type), qualified_name(value_type)
                )
            )
...
def check_number(argname: str, value, expected_type):
    if expected_type is complex and not isinstance(value, (complex, float, int)):
        raise TypeError(
            "type of {} must be either complex, float or int; got {} instead".format(
                argname, qualified_name(value.__class__)
            )
        )
    elif expected_type is float and not isinstance(value, (float, int)):
        raise TypeError(
            "type of {} must be either float or int; got {} instead".format(argname, qualified_name(value.__class__))
        )
...
def check_io(argname: str, value, expected_type):
    if expected_type is TextIO:
        if not isinstance(value, TextIOBase):
            raise TypeError(
                "type of {} must be a text based I/O object; got {} instead".format(
                    argname, qualified_name(value.__class__)
                )
            )
        ...
        if not isinstance(value, (RawIOBase, BufferedIOBase)):
            raise TypeError(
                "type of {} must be a binary I/O object; got {} instead".format(
                    argname, qualified_name(value.__class__)
                )
            )
    elif not isinstance(value, IOBase):
        raise TypeError(
            "type of {} must be an I/O object; got {} instead".format(argname, qualified_name(value.__class__))
        )
...
class TypeChecker:
    ...
    def should_check_type(self, func: Callable) -> bool:
        if not func.__annotations__:
            ...
            return any(
                func.__module__ == package or func.__module__.startswith(package + ".") for package in self._packages
            )
