...
class SemanticAnalysis(Visitor):
    ...
    def post_CallExpr(self, N: CallExpr) -> None:
        ...
        def compatible(func_type, arguments):
            """A helper function for type compatibility of function calls."""

            if len(func_type.param_types) != len(arguments):
                return False, "number of arguments do not match parameter list"

            for p, a in zip(func_type.param_types, arguments):
                if p != a.type:
                    return False, "param<->arg mismatch: {} <-> {}".format(p, a.type)

            return True, "Function is compatible"

        found_decl = None
        reasons = []
        for decl in N.callee.decl:
            matches, error = compatible(decl.type, arguments)
            if not matches:
                reasons.append((decl, error))
                continue

            if found_decl is not None:
                self.error(N, f"Ambigous function overload between\n  {found_decl}\nand\n  {decl}")

            found_decl = decl

        if not found_decl:
            msg = ""
            for decl, reason in reasons:
                msg += f"  {decl.name}={decl.type}: {reason}\n"
            self.error(N, "Could not resolve overloading, candidates were:\n" + msg)
            return

        # Override decl
        N.callee.decl = found_decl
        N.callee.type = found_decl.type
        N.type = decl.type.return_type
